...
select4 <- order(Pvars4, decreasing = TRUE)[seq_len(min(ntop, 
                                                        length(Pvars4)))]
PCA_retinaKO <- prcomp(t(scaled_counts_retinaKO[select4, ]), scale = F)
...
ggplot(dataGG4, aes(PC_retinaKO1, PC_retinaKO2, color=treatment)) +
  ...
  ylab(paste0("PC2: ",percentVar4[2],"% variance")) + labs(title = "Retina ABCA4KO PCA 1/2")

ggplot(dataGG4, aes(PC_retinaKO3, PC_retinaKO4, color=treatment)) +
  geom_point(size=3) +
  xlab(paste0("PC3: ",percentVar4[3],"% variance")) +
  ylab(paste0("PC4: ",percentVar4[4],"% variance")) + labs(title = "Retina ABCA4KO PCA 3/4")
```
